...
int main()
{
   long n,seed,seq,i,j,idum,meancount,dfac;
   double dt,tf,ts,energ,energ0,p0,theta0,pr,kin,potential,time,tcount,err,
          magx,magy,mean,st,sp,exp;
   FILE *arq1,*enrg,*ferr,*fmag, *favg, *pmom, *phasep;
...
/* Prepares initial conditions */
   condini(n,&idum,p0,theta0,r,p);
	...
	phasep = fopen("InitPhase--rep.dat","w");
...
/* Compute initial kinetic and potential energies */
   ...
   kinetic(&kin,n,p);
   kin=kin/(double) n;
   potential=-(1.0-magx*magx-magy*magy)/2.0;         /*Repulsive potential V -> -V*/
...
/*			Energy and Magnetization			*/
   ...
   sprintf(ener, "ENERGY-n_%li-p0_%g--REP", n,p0);
   sprintf(magne, "MAG-n_%li-p0_%g--REP", n,p0);  

/*			Phase Portraits			*/   
   sprintf(inic, "Initial-n_%li-p0_%g--REP", n,p0);
   sprintf(finc, "Final-n_%li-p0_%g--REP", n,p0);     
...
/* time loop */
   ...
   {    
      ...
      if (tcount>=ts)
         ...
         kinetic(&kin,n,p);
         kin=kin/(double) n;
         potential=-(1.0-magx*magx-magy*magy)/2.0;     /*Repulsive potential*/
         energ=kin+potential;
	...
	phasep = fopen("FinalPhase--rep.dat","w");
...
/*		Plotting		*/
  ...
  fprintf(pipe, "call 'exportPP.gp' '%s' 'InitPhase--rep.dat'\n",inic);
  fprintf(pipe, "call 'exportPP.gp' '%s' 'FinalPhase--rep.dat'\n",finc);    
  fclose(pipe);
